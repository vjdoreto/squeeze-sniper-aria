...
```
paper_closed.jsonl        → trades fechados com sinal completo (22 campos — fix 04/06/2026)
...
| Gate CVD `min_cvd_change_pct_no_cascade` | signal_engine.py L580, preferences.json | ✅ |
| Signal completo em paper_closed.jsonl | paper_tracker.py L793 | ✅ |

**Aguardando próxima sessão:**

| Item | Descrição |
|------|-----------|
| Verificar Kelly floor | Após liquidações funcionando, score muda e Kelly recalcula |
| Validar fix liquidações | Confirmar `liq_short_1m > 0` na próxima sessão de trades |
| Acumular 50+ trades | KPI mínimo para calibrar thresholds e avaliar GO/LIVE |
