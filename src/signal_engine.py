...
class SqueezeIgnition:
    ...
    def _write_ghost_signal(
        ...
        try:
            ...
            entry: Dict[str, Any] = {
                ...
                "ema_trend_1h": d.get("ema_trend:1h") or 0,
    ...
    def analyze(
        ...
        # F-18 (corrigido): EMA:4h bearish sozinho é suficiente para bloquear.
        # Segunda condição (exp_btc_norm_1h < -1.5) removida — anulava o gate.
        ...
        if (
            ...
            signal = {
                    ...
                    "cvd_streak": self._cvd_streak.get(symbol, 0),
                    "rsi_5m": rsi_5m or 0.0,
                    "rsi_1h": rsi_1h or 50.0,
                    "ema_trend": ema_tr or 0,
                    "ema_trend_4h": d.get("ema_trend:4h") or 0,
                    "ema_trend_1h": d.get("ema_trend:1h") or 0,
                    "ob_imbalance": d.get("ob_imbalance", 1.0) or 1.0,
                    "range_level": d.get("range_level:5m") or 0,
                    "score": score,
                    "timestamp": time.time(),
